# -*- coding: utf-8 -*-
"""
This code has been originally
adapted from 'https://keras.io/examples/generative/ddpm/'
"""
...
batch_size = 32
num_epochs = 200  # Just for the sake of demonstration
...
class DiffusionModel(keras.Model):
    ...
    def plot_images(
        ...
        for i, image in enumerate(generated_samples):
            if num_rows == 1:
                #print(image.shape)
                ax[i].imshow(image[:,:,np.random.randint(0,4)])
                ax[i].axis("off")
            else:
                #print(image.shape)
                ax[i // num_cols, i % num_cols].imshow(image[:,:,np.random.randint(0,4)])
                ax[i // num_cols, i % num_cols].axis("off")

        plt.tight_layout()
        plt.savefig('/home/mishra/img_gen/ddpm_sample.png')
        plt.show()
